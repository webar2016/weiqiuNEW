...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeImage]){
       ...
       // NSString *pictureDataString=[fileData base64Encoding];
      //  NSLog(@"str = %@",fileData);
        //保存图片至相册
      //  UIImageWriteToSavedPhotosAlbum(self.imageView.image, self, @selector(image:didFinishSavingWithError:contextInfo:), NULL);
        //上传图片
      //  [self uploadImageWithData:fileData];
//        NSDictionary *parameters = @{@"userId": @"29",@"newsType": @"1",@"comment": @"asadsad",@"topicId": @"1",};
//        NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:@"POST" URLString:@"http://121.40.132.44:92/tq/setComment" parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
//           // [formData appendPartWithFileURL:[NSURL fileURLWithPath:@"file://path/to/image.jpg"] name:@"file" fileName:@"filename.jpg" mimeType:@"image/jpeg" error:nil];
//            [formData appendPartWithFileData:fileData name:@"dsad" fileName:@"dsadad" mimeType:@"image/jpeg"];
//        } error:nil];
//        
...
//        [uploadTask resume];
        ...
        [parameters setObject:@"9755@qq.com" forKey:@"comment"];
        [parameters setObject:@"1" forKey:@"topicId"];
//        AFHTTPSessionManager *manager = [AFHTTPSessionManager manager];
//        manager.responseSerializer = [AFHTTPResponseSerializer serializer];
//        [manager POST:[NSString stringWithFormat:@"http://121.40.132.44:92/tq/setComment?userId=29&newsType=1&comment=%@%@%@&topicId=1",@"zfczczc",IMAGE,@"fdsfsdf" ] parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
//             [formData appendPartWithFileData:fileData name:@"dsad" fileName:@"dsadad.jpg" mimeType:@"image/jpeg"];
//        } progress:^(NSProgress * _Nonnull uploadProgress) {
//            
