...
-(void)addCollect:(id)model
{
   ...
   // NSString *insertSql = @"insert into bigarea(areaId,areaName,isCountry) values (@"",?,?)";
    ...
    NSString *insertSql = [NSString stringWithFormat:@"insert into bigarea (areaId, areaName, isCountry) values ('%ld', '%@', '%ld'),('123', 'werrr', '123')", (long)cItem.areaId,cItem.areaName ,(long)cItem.isCountry];
   // 图片转化成二进制
