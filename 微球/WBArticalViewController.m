...
-(void)loadData{
    NSString *url = [NSString stringWithFormat:@"%@/tq/getCommentById?commentId=%ld",WEBAR_IP,(long)self.commentId];
...
-(void)upLoadLikeTap{
    [MyDownLoadManager getNsurl:[NSString stringWithFormat:@"%@/tq/topicPraise?commentId=%ld&userId=%@&toUserId=%ld",WEBAR_IP,(long)self.commentId,[WBUserDefaults userId],(long)self.userId] whenSuccess:^(id representData) {
